...
export class MainContract implements Contract {
    ...
    static createFromConfig(config: MainContractConfig, code: Cell, workchain = 0) {
        const data = mainContractConfigToCell(config);//beginCell().endCell();
    ...
    async sendInternalMessage(provider: ContractProvider, sender: Sender, value: bigint, incr: number) {
        const body = beginCell().storeUint(1, 32).storeUint(incr, 32).endCell();
